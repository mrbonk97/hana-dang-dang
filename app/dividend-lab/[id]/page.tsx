...
const DividendLabDetailPage = async ({ params }: Props) => {
  const accountId = params.id.replaceAll("-", "");
  const data1 = await getAccountStockApi(accountId);
  const data2 = await getAccountDividendHistoryApi(accountId);
...
export default DividendLabDetailPage;
